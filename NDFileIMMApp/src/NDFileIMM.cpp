...
/** Sets pasynUser->reason to one of the enum values for the parameters defined
 * for the NDPluginFile class if the drvInfo field matches one the strings
 * defined for it.
 * If the parameter is not recognized by this class then calls
 * NDPluginDriver::drvUserCreate.
 * Uses asynPortDriver::drvUserCreateParam.
 * \param[in] pasynUser pasynUser structure that driver modifies
 * \param[in] drvInfo String containing information about what driver function
 *      is being referenced
 ...
 * \return Returns asynSuccess if a matching string was found, asynError if
 *      not found. */
...
/*asynStatus NDFileIMM::drvUserCreate(asynUser *pasynUser,
 ...
 if (status) status = NDPluginFile::drvUserCreate(pasynUser, drvInfo,
                         pptypeName, psize);
 return(status);
 }
 ...
 * maximum length which we define here. */
#define MAX_ATTRIBUTE_STRING_SIZE 256

/** This is called to open a TIFF file.
 */
asynStatus NDFileIMM::openFile(const char *fileName, NDFileOpenMode_t openMode,
        NDArray *pArray) {
    ...
    //
    // if filename is NULL, then we don't open a file on filesystem, but we
    // will pretend we are writing to files,. this is for computing IMM format
    // without actuallty writing to disk.
    //
...
//	printf("FileName: %s\n", fileName);
    /* Create the file. */
//!! we shoudl override CDPluginDriver:;, NDPludFile, openBase, Writebase and
    //createFileName functions to take care of filenumbers and filenames.
    ...
    // this is a kludge because base class uincs the NDFileNumber for us. we
    // are negating that so we can keep track ourselves here.
...
// keep track of filenumber so if base class messes it up we are not consused...
    ...
    cf->open_w(str0, str1, fn0, fn0 + fn1 - 1);
//			cf->open_w(fileName,-1,-1);
    ...
    if (!is_open_good) {
        ...
        setStringParam(NDFileWriteMessage,
                "Error: Could not open file. Check path.");
        return (asynError);
    } else {
...
/** This is called to write data a single NDArray to the file.  It can be
 * called multiple times to add arrays to a single file in stream or
 * capture mode
 * */
asynStatus NDFileIMM::writeFile(NDArray *pArray) {
    ...
    if (is_throw_frame) {
        ...
        // we add 2000 for the header.. it is act only 1024, but we get extra
        // room...
        dimSizeOut[0] = (size_t) 1;
        dimSizeOut[1] = this->max_imm_bytes;
        my_array = this->pNDArrayPool->alloc(2, dimSizeOut, NDUInt8, (size_t) 0,
                (void*) 0);

        int one = 1;

        //add new attr to img, if not already there. if there, it updates values
        my_array->pAttributeList->add("is_already_imm",
                "if 1 then this is imm data", NDAttrInt32, &one);
        ...
        if (my_array == 0) {
            ...
            setIntegerParam(NDFileIMM_NmissedTimeStamps,
...
//NDFileFormat
    ...
    if (is_already_imm) {
        ...
        cf->saveFileIMMRaw(file_coreco_ts, file_elapsed_ts, file_systick_ts, //timestamp- ise ndarray param
                threshold, //thresh
                sizex, sizey,
                bytesperpix, // bytes per pix-00 need to get this from NDArray... use short for now
                pArray->pData, this->nextRecord, //fiklenumber- where do we get it?
        ...
        if (is_throw_frame) {
            if (cf->last_nbytes <= my_array->dataSize) {
                ...
                printf("Error- Could not throw IMM to plugins- not enough "
                        "mem\n");
                setIntegerParam(NDFileWriteStatus, NDFileWriteError);
                setStringParam(NDFileWriteMessage,
                        "Error- Could not throw IMM to plugins- not enough "
                        "mem.");
            }
            callParamCallbacks();
        ...
        cf->saveFileIMMComp(file_coreco_ts, file_elapsed_ts, file_systick_ts,
                threshold, //thresh
                sizex, sizey, bytesperpix, // bytes per pix-00 need to get this from NDArray... use short for now
                pArray->pData, this->nextRecord, //fiklenumber- where do we get it?
                cam_type, // camtype should be a ndarray param
                acq_time, //acq period- do we read the param or use an ndarray param?
                &imm_pixels);
...
// dbpf "SimMaddog:IMM:EnableCallbacks","Enable"
    ...
    if ((pArray->uniqueId - last_unique_id) > 1) {
        setIntegerParam(NDFileIMM_NmissedIDs,
...
//	printf("Num Attributes %i \n", numAttributes);
    ...
    for (attrCount = 0; attrCount < numAttributes; attrCount++) {
        ...
        if (strcmp(name, "is_already_imm") == 0) {
            pAttribute->getValue(attrDataType, (void*) &is_already_imm,
                    attrSize);
            if (is_at_msg)
 ...
 * Catch parameter changes.  If the user changes the path or name of the
 * template file, load the new template file.
...
int NDFileIMM::recursePath(char *pathstr, bool is_makedirs) {
    ...
    while (slash_pos != string::npos) {
        // path up to / not incl /
        //here we are actually calling the copy constructor...of part path.
        // The returned str from substr?
        // prob in memory inside fullpath,,, it will go out of scope and
        // destruct?
        part_path = full_path.substr(start, slash_pos);

        // this part of path... say path is D:/aaa/bbb/ccc   then worrd is
        // bbb for example. If loas_slash_pos is 0, then the path starts with /.
        // this is special case. make sure pathword has the 1st /
        if (full_path[last_slash_pos] == '/')
            path_word = full_path.substr(last_slash_pos + 1,
                    (slash_pos - last_slash_pos - 1));
        else
            path_word = full_path.substr(last_slash_pos,
                    (slash_pos - last_slash_pos));
            ...
            if (!is_makedirs) {
                ...
                if (status != 0) {
                    ...
                    if (status != 0) {
                        ...
                        setStringParam(NDFileWriteMessage,
                                "ERROR-Cound not create Directory");

                        return (-128 - part_number);
                    } else
                        printf("Created Dir %s\n", part_path.c_str());
                    setStringParam(NDFileWriteMessage,
                            "INFO-Created New Directory");
...
extern "C" int drvNDFileIMMConfigure(const char *portName, int max_imm_bytes,
        int queueSize, int blockingCallbacks, const char *NDArrayPort,
        int NDArrayAddr, int priority, int stackSize) {
    NDFileIMM *pPlugin = new NDFileIMM(portName, max_imm_bytes, queueSize,
            blockingCallbacks, NDArrayPort, NDArrayAddr, priority, stackSize);
    //  return(asynSuccess);
...
static const iocshArg * const initArgs[] = { &initArg0, &initArg01, &initArg1,
        &initArg2, &initArg3, &initArg4, &initArg5, &initArg6 };
...
#if ADCORE_VERSION>2
NDFileIMM::NDFileIMM(const char *portName,int max_imm_bytes ,int queueSize,
...
NDFileIMM::NDFileIMM(const char *portName, int max_imm_bytes, int queueSize,
        ...
        NDPluginFile(portName, queueSize, blockingCallbacks, NDArrayPort,
                NDArrayAddr, 1, num_params, 500, 0, asynGenericPointerMask,
...
#endif
{
    ...
    paramStrings[0] = new param_type_str(&NDFileIMM_threshold, asynParamInt32,
            "NDFileIMM_threshold");
    paramStrings[1] = new param_type_str(&NDFileIMM_is_imm_comp, asynParamInt32,
            "NDFileIMM_is_imm_comp");
    paramStrings[2] = new param_type_str(&NDFileIMM_num_imm_pixels,
            asynParamInt32, "NDFileIMM_num_imm_pixels");
    paramStrings[3] = new param_type_str(&NDFileIMM_timestamp, asynParamFloat64,
            "NDFileIMM_timestamp");
    paramStrings[4] = new param_type_str(&NDFileIMM_uniqueID, asynParamInt32,
            "NDFileIMM_uniqueID");
    paramStrings[5] = new param_type_str(&NDFileIMM_printAttributes,
            asynParamInt32, "NDFileIMM_printAttributes");
    paramStrings[6] = new param_type_str(&NDFileIMM_NmissedTimeStamps,
            asynParamInt32, "NDFileIMM_NmissedTimeStamps");
    paramStrings[7] = new param_type_str(&NDFileIMM_framePeriod,
            asynParamFloat64, "NDFileIMM_framePeriod");
    paramStrings[8] = new param_type_str(&NDFileIMM_NmissedIDs, asynParamInt32,
            "NDFileIMM_NmissedIDs");

    paramStrings[9] = new param_type_str(&NDFileIMM_Nimg_rst_ts, asynParamInt32,
            "NDFileIMM_Nimg_rst_ts");
    paramStrings[10] = new param_type_str(&NDFileIMM_throw_images,
            asynParamInt32, "NDFileIMM_throw_images");

    paramStrings[11] = new param_type_str(&NDFileIMM_fileevent, asynParamInt32,
            "NDFileIMM_fileevent");

    paramStrings[12] = new param_type_str(&NDFileIMM_is_already_imm,
            asynParamInt32, "NDFileIMM_is_already_imm");
    paramStrings[13] = new param_type_str(&NDFileIMM_imm_systicks,
            asynParamInt32, "NDFileIMM_imm_systicks");
    paramStrings[14] = new param_type_str(&NDFileIMM_imm_corecoticks,
            asynParamInt32, "NDFileIMM_imm_corecoticks");
    paramStrings[15] = new param_type_str(&NDFileIMM_imm_elapsed,
            asynParamFloat64, "NDFileIMM_imm_elapsed");
    paramStrings[16] = new param_type_str(&NDFileIMM_imm_dlen, asynParamInt32,
            "NDFileIMM_imm_dlen");

    paramStrings[17] = new param_type_str(&NDFileIMM_format, asynParamInt32,
            "NDFileIMM_format");
